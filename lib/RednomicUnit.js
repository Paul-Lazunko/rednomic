...
export class RednomicUnit {
  constructor(options) {
    RednomicOptionsChecker.checkOptions(options, "unit");
    ...
    this.connector = new RednomicUnitConnector({ server: options.redisServer, timeout: this.options.requestTimeout });
  ...
  async handler(data, key) {
    ...
    let context = {
      call: this.connector.call.bind(this.connector),
      log: (type, data) => {
    ...
    if (this.options.service.constructor.name === RednomicConstants.types.asyncFunction) {
      try {
        ...
        this.waitStreams(id, this.resolver.bind(this, result, key));
      } catch (e) {
        this.waitStreams(id, this.resolver.bind(this, e, key));
      }
    } else {
  ...
  waitStreams(id, cb) {
    if (this.backStreams && Array.isArray(this.backStreams[id]) && this.backStreams[id].length) {
      let interval = setInterval(() => {
        if (!this.backStreams[id].length) {
