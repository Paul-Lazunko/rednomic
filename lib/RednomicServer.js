...
export class RednomicServer {
  ...
  use (unitId, data, req, res, pipe = false ) {
    let id = RednomicQueueId(), ctx = { req, res, id };
    this.queue.push(ctx);
    if ( pipe ) {
      this.adapter.pipe(`${unitId}-stream-${id}`, req, this.streamsMaxAge, this.resolver.bind(this,[unitId, id,  data]))
    } else {
